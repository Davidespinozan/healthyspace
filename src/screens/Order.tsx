...
import { Check, Clock, ChefHat, PackageCheck, PartyPopper, X } from 'lucide-react';
import { useStore, type OrderStatus } from '../state/store';
import { money } from '../components/ui';

const STEPS: { id: OrderStatus; label: string; sub: string; Icon: typeof Check }[] = [
  { id: 'recibido', label: 'Pedido recibido', sub: 'Ya lo tenemos', Icon: Check },
  { id: 'preparando', label: 'Preparando', sub: 'Lo estamos armando fresco', Icon: ChefHat },
  { id: 'listo', label: 'Listo para recoger', sub: 'Escanea tu QR en el truck', Icon: PackageCheck },
  { id: 'recogido', label: 'Recogido', sub: '¡Buen provecho!', Icon: PartyPopper },
];
...
export default function Order() {
  ...
  useEffect(() => {
    if (!order) return;
    QRCode.toDataURL(`HSPACE:${order.code}`, {
      margin: 1, width: 460, color: { dark: '#0E2521', light: '#00000000' },
    }).then(setQr).catch(() => setQr(''));
  }, [order?.code]);

  // Avance simulado del pedido (demo): recibido → preparando → listo.
  useEffect(() => {
    if (!order || order.status === 'listo' || order.status === 'recogido') return;
    ...
    <div className="page dark-depth" style={{ minHeight: '100vh', background: 'var(--forest)', color: 'var(--on-dark)' }}>
      <div style={{ display: 'flex', alignItems: 'center', padding: 'calc(14px + env(safe-area-inset-top,0)) 20px 8px' }}>
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: 13, color: 'var(--on-dark-2)', marginTop: 3 }}>
      ...
      <div style={{ padding: '18px 20px 4px', display: 'grid', placeItems: 'center' }}>
        <div style={{
          ...
          boxShadow: ready ? '0 0 0 3px var(--amber), var(--sh-xl)' : 'none',
        }}>
          ...
          <div style={{ textAlign: 'center', color: ready ? 'var(--forest)' : 'var(--on-dark-2)' }}>
            <div style={{ fontWeight: 800, fontSize: 15 }}>
              {ready ? 'Muéstralo en el food truck' : 'Tu QR se activa cuando esté listo'}
            </div>
        ...
        {!ready && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginTop: 16, color: 'var(--amber)' }}>
            <Clock size={15} /> <b className="tabular" style={{ fontSize: 13.5 }}>Listo en ~{order.pickupMin} min</b>
          </div>
        )}
      </div>

      {/* Stepper */}
      <div style={{ padding: '22px 24px 40px', display: 'grid', gap: 0 }}>
        {STEPS.map((s, i) => {
      ...
      <div style={{ padding: '0 20px calc(24px + var(--safe-b))' }}>
